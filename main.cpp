...
int main () {
    size_t countOfCardsInDeck = 0;
    
    while (countOfCardsInDeck < 1 || countOfCardsInDeck > 52) {
        try {
            std::cout << "countOfCardsInDeck: ";
            getNumberS(countOfCardsInDeck);
        } catch (const std::exception & mess) {
            std::cerr << mess.what() << std::endl;
        }
    }
    
    std::cout << std::endl;
    
    Deck deck = Deck(countOfCardsInDeck, 10, Hearts);
    int com;
    bool exit = false;

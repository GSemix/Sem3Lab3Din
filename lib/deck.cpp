...
Cards::Cards (const Cards & card): num(card.num), suit(card.suit) {} // Создание экземпляра класса с его инициализацией другим экземпляром класса (копирующий конструктор)
...
Deck::Deck () {
    deck = new Cards[countOfCardsInDeck];
}

Deck::Deck (size_t newCountOfCardsInDeck) {
    countOfCardsInDeck = newCountOfCardsInDeck;
    
    deck = new Cards[countOfCardsInDeck];
}

Deck::Deck (size_t newCountOfCardsInDeck, int num, suitCard suit) {
    countOfCardsInDeck = newCountOfCardsInDeck;
...
void Deck::addCard (size_t numberOfCardInDeck, const Cards & test) throw(size_t, const std::exception &) { // Создает или перезаписывает карту по индексу numberOfCardInDeck
    if (numberOfCardInDeck < 0 || numberOfCardInDeck > (countOfCardsInDeck - 1)) {
        throw numberOfCardInDeck;
    }
